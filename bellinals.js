...
const bellcore = require('./bitcore-lib-bells')
const axios = require('axios')
const fs = require('fs')
const dotenv = require('dotenv')
const mime = require('mime-types')
const express = require('express')
const { PrivateKey, Address, Transaction, Script, Opcode } = bellcore
const { Hash, Signature } = bellcore.crypto

dotenv.config()

if (process.env.TESTNET == 'true') {
    bellcore.Networks.defaultNetwork = bellcore.Networks.testnet
}
...
async function walletSync() {
    ...
    console.log('syncing utxos with bellsd rpc')

    const body = {
        jsonrpc: "1.0",
        method: "listunspent",
        id: "1",
        parameter: [1, 9999999, [`${wallet.address}`]],
      };
...
async function extract(txid) {
    ...
    let resp = await axios.post(process.env.NODE_RPC_URL, body, options)
    let result = resp.data.result
    let script = Script.fromHex(result.vin[0].scriptSig.hex)
    console.log(script)
    ...
    while (remaining && chunks.length) {
        ...
        if (n !== remaining - 1) {
            txid = result.vout[0].spent.hash //to check for multi txn inscription
            resp = await axios.post(process.env.NODE_RPC_URL, body, options);
            result = resp.data.result
            script = Script.fromHex(result.vin[0].scriptSig.hex)
